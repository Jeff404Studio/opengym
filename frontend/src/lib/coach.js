...
// account, but it does have a canned provider (lib/coach-demo.js), and hiding the app's most
 ...
 * apart. If you change one, change both; coach.test.js checks them against shared fixtures.
 */
...
const CHANGE_APPLY = {
  ...
  'swap-exercise': (s, c) => {
    ...
    r.ex[i] = { ...old, id: a.id, ...(a.sets ? { sets: a.sets } : {}), ...(a.reps ? { reps: a.reps } : {}), ...(a.weight > 0 ? { weight: a.weight } : {}) }
  },
...
export const exName = id => {
  const ex = EXIDX[id]
  return ex ? nameFor(ex) : t('Unknown exercise')
}
